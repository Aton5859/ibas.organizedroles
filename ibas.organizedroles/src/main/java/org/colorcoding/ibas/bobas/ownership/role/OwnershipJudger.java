package org.colorcoding.ibas.bobas.ownership.role;
...
import org.colorcoding.ibas.bobas.organization.role.OrgUser;
import org.colorcoding.ibas.bobas.organization.role.OrganizationManager;
import org.colorcoding.ibas.bobas.organization.role.OrganizationalRelationship;
	...
	public boolean canRead(IDataOwnership bo, IUser user) {
		...
		if (user != null && bo != null) {
			...
			if (user != OrganizationFactory.SYSTEM_USER) {// 系统用户，不进行权限处理
				// 操作用户是否有权限读取数据
				IOwnership ownership = this.getOwnership(bo.getObjectCode(), ((OrgUser) user).getCode());
				if (ownership != null) {
				...
				if (canRead && bo.getDataOwner() != null) {
					...
					if (owner != null && !owner.equals(user)) {
						...
						ownership = this.getOwnership(bo.getObjectCode(), ((OrgUser) owner).getCode());
						if (ownership != null) {
							// 数据所有者配置了权限
