...
export class OrganizationalStructureListApp extends ibas.BOListApplication<IOrganizationalStructureListView, bo.OrganizationalStructure> {
    ...
    constructor() {
        ...
        this.boCode = OrganizationalStructureListApp.BUSINESS_OBJECT_CODE;
    ...
    /** 删除数据，参数：目标数据集合 */
    protected deleteData(data: bo.OrganizationalStructure): void {
        // 检查目标数据
        if (ibas.objects.isNull(data)) {
            this.messages(ibas.emMessageType.WARNING, ibas.i18n.prop("sys_shell_please_chooose_data",
        ...
        let beDeleteds: ibas.ArrayList<bo.OrganizationalStructure> = new ibas.ArrayList<bo.OrganizationalStructure>();
        if (data instanceof Array) {
            for (let item of data) {
                if (ibas.objects.instanceOf(item, bo.OrganizationalStructure)) {
                    item.delete();
                    beDeleteds.add(item);
                }
            }
        ...
        this.messages({
            ...
            onCompleted(action: ibas.emMessageAction): void {
                if (action === ibas.emMessageAction.YES) {
                    try {
                        let boRepository: BORepositoryOrganizedRoles = new BORepositoryOrganizedRoles();
                        let saveMethod: Function = function (beSaved: bo.OrganizationalStructure): void {
